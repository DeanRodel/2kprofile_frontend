...
const SetDivisionStats = ({ setPlayer, setDefaultGame }) => {
  ...
  const setAccount = () => {
    ...
    const data = {
      ...
      platform: document.getElementById("divisionPlatform").value,
    };
    axios
      .post("/api/server/setDivisionStats", data)
      .then((res) => {
        setSuccess(res.data.success);
        setPlayer(res.data.player);
        if (res.data.player.stats.games.length === 1) {
          setDefaultGame("division");
        }
        setLoading(false);
      })
      .catch((err) => {
...
export default SetDivisionStats;
