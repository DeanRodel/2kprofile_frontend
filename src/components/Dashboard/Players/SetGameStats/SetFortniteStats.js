...
const SetFortniteStats = ({ setPlayer, setDefaultGame }) => {
  ...
  const setAccount = () => {
    ...
    const data = {
      name: document.getElementById("fortniteName").value,
    };
    axios
      .post("/api/server/setFortniteStats", data)
      .then((res) => {
        setSuccess(res.data.success);
        setPlayer(res.data.player);
        if (res.data.player.stats.games.length === 1) {
          setDefaultGame("fortnite");
        }
        setLoading(false);
      })
      .catch((err) => {
...
export default SetFortniteStats;
