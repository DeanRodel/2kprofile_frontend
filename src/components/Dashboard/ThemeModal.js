...
export default function ThemeModal({ hideModal, themes, api }) {
  const [index, setIndex] = useState(0);
  const [success, setSuccess] = useState("");

  const next = () => {
    setIndex(index + 1 >= themes.length ? 0 : index + 1);
    setSuccess("");
  };

  const prev = () => {
    setIndex(index - 1 < 0 ? themes.length : index - 1);
    setSuccess("");
  };

  function updateTheme() {
    let body = { theme: themes[index] };
            ...
            <div className="sm:flex sm:items-center justify-center">
              <div className="mt-3 text-center s">
                <div className="mt-2 flex justify-center">
                  <button
                    className="text-black w-10 cursor-pointer"
                    onClick={prev}
                  >
                    <svg
                  ...
                  <img
                    src={themes[index].background}
                    id="templateImg"
                    alt={`${themes[index].name}`}
                    style={{
                      width: 400,
                      height: 200,
                ...
                <p className="capitalize">{themes[index].name}</p>
              </div>
            </div>
          </div>
